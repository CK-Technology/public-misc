...
function Invoke-BluebeamDiagnostic {
    ...
    $logRoot = 'C:\ProgramData\CKScripts\Logs'
    ...
    # Versions.txt" shipped inside MSIBluebeamRevu21.10.0x64.zip. Releases at or
    # below 21.0.15 are not enumerated here.
    ...
    if ($userDataProducts.Count -eq 0) {
        Write-Report '  (none)'
        $findings.Add('Zero Windows Installer product registrations. This is the exact condition that trips bluebeamRecovery.ps1 line 345 ("found 0. Refusing an ambiguous recovery.") - the guard treats 0 the same as an ambiguous 2+.')
    }
    ...
    foreach ($path in @(
            ...
            'C:\ProgramData\CKScripts\BluebeamPackages')) {
    ...
    Write-Report '  This script changed nothing. Send the log above before any'
    Write-Report '  remediation is attempted:'
    Write-Report "      $logPath"
    Write-Report ''
    Write-Report '  Do NOT re-run bluebeamRecovery.ps1 or bluebeamUpdates.ps1 on this'
    Write-Report '  machine. Both refuse at their "exactly one product" guard when the'
    Write-Report '  count is 0, and neither reads the UpgradeCodes hive where the'
    Write-Report '  blocking residue lives.'
}
